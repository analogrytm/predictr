/**
  A data model representing a user on HaveFunHub
...
**/
  ...
  HaveFunHub.User = HaveFunHub.Model.extend({

    avatarLarge: (function() {
      return 'avatarLarge';//HaveFunHub.Utilities.avatarUrl(this.get('username'), 'large', this.get('avatar_template'));
    }).property('username'),

    avatarSmall: (function() {
      return 'avatarSmall';//HaveFunHub.Utilities.avatarUrl(this.get('username'), 'small', this.get('avatar_template'));
    }).property('username'),
    ...
    trustLevel: (function() {
      return HaveFunHub.get('site.trust_levels').findProperty('id', this.get('trust_level'));
    ...
    copy: function(deep) {
      return HaveFunHub.User.create(this.getProperties(Ember.keys(this)));
    },
    ...
    filterStream: function(filter) {
      if (HaveFunHub.UserAction.statGroups[filter]) {
        filter = HaveFunHub.UserAction.statGroups[filter].join(",");
      }
      ...
      return $.ajax({
        ...
        success: function(result) {
          if (result) {
            ...
            action = Em.A();
            action.pushObject(HaveFunHub.UserAction.create(result));
            action = HaveFunHub.UserAction.collapseStream(action);
        ...
        success: function(result) {
          ...
          if (result && result.user_actions && result.user_actions.each) {
            ...
            result.user_actions.each(function(i) {
              return copy.pushObject(HaveFunHub.UserAction.create(i));
            });
            copy = HaveFunHub.UserAction.collapseStream(copy);
    ...
    inboxCount: (function() {
      ...
      r = 0;
      this.get('stats').each(function(s) {
        if (s.action_type === HaveFunHub.UserAction.GOT_PRIVATE_MESSAGE) {
          r = s.count;
          return false;
        }
      });
    ...
    sentItemsCount: (function() {
      ...
      r = 0;
      this.get('stats').each(function(s) {
        if (s.action_type === HaveFunHub.UserAction.NEW_PRIVATE_MESSAGE) {
          r = s.count;
          return false;
        }
      });
  ...
  HaveFunHub.User.reopenClass({
    ...
    groupStats: function(stats) {
      ...
      stats.each(function(s) {
        ...
        found = false;
        _ref = HaveFunHub.UserAction.statGroups;
        for (k in _ref) {
          v = _ref[k];
      ...
      $.ajax({
        ...
        success: function(json) {
          ...
          json.user.stats = _this.groupStats(json.user.stats.map(function(s) {
            ...
            obj.isPM = obj.action_type === HaveFunHub.UserAction.NEW_PRIVATE_MESSAGE || obj.action_type === HaveFunHub.UserAction.GOT_PRIVATE_MESSAGE;
            return obj;
          }));
          if (json.user.stream) {
            json.user.stream = HaveFunHub.UserAction.collapseStream(json.user.stream.map(function(ua) {
              return HaveFunHub.UserAction.create(ua);
            }));
          }
          user = HaveFunHub.User.create(json.user);
